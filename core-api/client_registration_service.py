...
from local_gsm_sms_service import send_verification_sms, verify_sms_code, send_notification_sms
...
class ClientRegistrationService:
    ...
    async def verify_sms_code(self, registration_id: str, sms_code: str) -> Dict[str, Any]:
        ...
        try:
            with Session(self.engine) as session:
                ...
                # Verify code using real SMS service
                verification_result = await verify_sms_code(registration.phone_number, sms_code)
                ...
                if not verification_result['success']:
                    ...
                    return {
                        "success": False,
                        "error": verification_result.get('error', 'Invalid SMS code'),
                        "code": "INVALID_CODE",
    ...
    async def _send_verification_sms(self, phone_number: str, sms_code: str, registration_id: UUID):
        ...
        Args:
            ...
            sms_code: Verification code to send (will be regenerated by SMS service)
            registration_id: Registration ID for tracking
        """
        try:
            # Use real SMS service which handles provider selection and database storage
            ...
            if result['success']:
                ...
                # Update the registration with the actual code from SMS service
                with Session(self.engine) as session:
                    registration = session.get(ClientRegistration, registration_id)
                    if registration:
                        registration.sms_code = result['verification_code']
                        session.commit()
